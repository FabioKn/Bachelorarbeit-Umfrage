-- KORRIGIERTES Supabase SQL Schema für Bachelorarbeit-Umfrage
-- Diese Befehle in der Supabase-Konsole unter "SQL Editor" ausführen
-- WICHTIG: Zuerst supabase-cleanup.sql ausführen, falls Tabellen bereits existieren

-- 1. KORRIGIERTE Video-Fragebogen Tabelle mit ALLEN benötigten Feldern
CREATE TABLE IF NOT EXISTS video_questionnaire (
  id BIGSERIAL PRIMARY KEY,
  participant_id UUID REFERENCES participants(id) ON DELETE CASCADE,
  
  -- Bestehende Fragebogen-Felder (aus questionnaire übernommen)
  gs_known BOOLEAN NOT NULL,
  vis_experience INTEGER NOT NULL CHECK (vis_experience >= 1 AND vis_experience <= 5),
  been_to_library BOOLEAN NOT NULL,
  ...
  memorable_book TEXT,
  shelf_levels INTEGER CHECK (shelf_levels >= 0),
  sofa_color TEXT,
  ...
  estimated_distance_video DECIMAL(10,2) CHECK (estimated_distance_video >= 0),
  
  timestamp TIMESTAMPTZ DEFAULT NOW(),
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 2. Erkundungssitzungen Tabelle (unverändert)
...
-- 3. KORRIGIERTE Finaler Fragebogen Tabelle mit ALLEN benötigten Feldern
CREATE TABLE IF NOT EXISTS final_questionnaire (
  ...
  memorable_book_final TEXT,
  shelf_levels_final INTEGER CHECK (shelf_levels_final >= 0),
  
  -- Sofa-Position Fragen
  ...
  estimated_distance DECIMAL(10,2) CHECK (estimated_distance >= 0), -- Distanzschätzung aus dem finalen Fragebogen
...
-- 5. Policies für anonymen Zugriff
...
-- 7. Trigger für automatische Zeitstempel-Updates
...
SELECT 
  ...
  fq.sofa_location,
  fq.overall_rating,
  fq.estimated_distance as final_estimated_distance
FROM participants p
...
SELECT 'Korrigierte Tabellen und Policies erfolgreich erstellt!' as status;
